  ...
  return <div className="app-shell">
    ...
    <div className="main-shell">
      ...
      <main className="content">
        ...
        {page === 'customer' && <CustomerPage onNavigate={go} />}
        {page === 'negotiation' && <NegotiationWorkspace onNavigate={go} />}
...
function Sidebar({ active, open, onNavigate, onClose }) {
  ...
  return <aside className={`sidebar ${open ? 'is-open' : ''}`}>
    <div className="brand"><div className="brand-mark"><img className="brand-logo" src="/dealflow360-logo.png" alt="DealFlow360 logo" /></div><div><strong>DealFlow360</strong><small>Revenue Ops Core</small></div><button className="icon-button close-nav" onClick={onClose} aria-label="Close navigation"><X size={17} /></button></div>
...
function DealDetailPage({ onNavigate }) {
  const goBack = () => onNavigate('health');
  return <>
...
function RescuePage({ onNavigate }) {
  ...
  return <>
    <div className="detail-backbar"><button className="back-button" onClick={() => onNavigate('deal-detail')}><ArrowLeft size={15} /> Back to deal intelligence</button><span>Rescue workspace / Helios Health System</span></div>
    ...
    <div className="metrics"><Metric label="Open rescue cases" value="8 Deals" detail="3 critical cases require action today" badge="Immediate" tone="red" /><Metric label="Revenue at risk" value="₹21.65 Cr" detail="Across 14 active intervention plans" badge="-8.4% WoW" tone="amber" /><Metric label="Protected this quarter" value="₹12.40 Cr" detail="8 deals successfully rescued" badge="+28%" tone="teal" /><Metric label="Rescue status" value={rescueStatus} detail={completedAction || 'Select a playbook action to begin'} badge={rescueStatus === 'Action in progress' ? 'Live' : 'Needs action'} tone={rescueStatus === 'Action in progress' ? 'teal' : 'red'} /></div>
    <Section title="Rescue workflow" icon={Zap}><div className="rescue-flow">{[['Risk', 'Critical health score · 48 / 100'], ['Cause', 'Warehouse shortage in Mumbai WH-01'], ['Impact', 'Clinical pilot milestone at risk'], ['Recommendation', 'Reallocate inventory and defend margin'], ['Action', rescueStatus], ['Outcome', completedAction ? 'Tracking intervention result' : 'Pending execution']].map(([label, detail], index) => <div className={`rescue-stage ${index === 4 && rescueStatus === 'Action in progress' ? 'current' : ''}`} key={label}><b>{index + 1}</b><strong>{label}</strong><small>{detail}</small></div>)}</div></Section>
    <Section title="Active rescue case" icon={AlertTriangle} action={<span className="status critical">Critical alert · 48</span>}><div className="rescue-case"><div className="rescue-hero"><button className="deal-summary-link" onClick={() => onNavigate('deal-detail')}><span className="case-label">Helios Health System · Enterprise Diagnostics Cloud</span><ArrowRight size={15} /></button><h2>Clinical deployment is blocked</h2><p>OptiSensor Pro-4 has a 14-day backorder in WH-01, directly stalling the pilot milestone.</p><div className="alert-box"><AlertTriangle size={16} /><span>Business impact: ₹7,47,60,000 deal value exposed and 4.8 margin points below target.</span><strong>48 / 100</strong></div><button className="secondary-button" onClick={() => onNavigate('deal-detail')}>Open deal detail</button></div><div className="rescue-evidence"><div><h3>Root causes</h3><div className="signal-list"><Signal title="Warehouse shortage" detail="6 units unavailable in Mumbai facility" tone="red" /><Signal title="Margin concession pressure" detail="Procurement requested ₹8,16,000 discount" tone="amber" /><Signal title="Stakeholder stagnation" detail="VP of IT inactive across two check-ins" /></div></div><div className="impact-callout"><small>Recommended playbook</small><strong>Protect timeline, preserve value</strong><p>Reallocate inventory from Dallas WH-02, pair it with a three-year warranty addendum, and re-anchor the CIO sponsor.</p></div></div></div></Section>
    <Section title="Recommended playbook" icon={Sparkles} action={completedAction && <span className="status success">{completedAction} started</span>}><div className="playbook rescue-playbook"><ActionRow number="1" title="Eliminate logistics backorder" detail="Reallocate 6 OptiSensor units from Dallas WH-02 within 24 hours." button={completedAction === 'Inventory reallocation' ? 'Started' : 'Execute'} onClick={() => runAction('Inventory reallocation')} /><ActionRow number="2" title="Defend contract gross margin" detail="Draft a counter-offer with a three-year warranty addendum." button={completedAction === 'Margin defense' ? 'Started' : 'Draft'} onClick={() => runAction('Margin defense')} /><ActionRow number="3" title="Re-anchor executive alignment" detail="Schedule a sponsor briefing with the Helios CIO this week." button={completedAction === 'Executive alignment' ? 'Started' : 'Schedule'} onClick={() => runAction('Executive alignment')} /></div></Section>
    <RiskTable onNavigate={onNavigate} />
  </>;
}
<<<<<<< HEAD
...
=======
function CustomerPage({ onNavigate }) { return <>
  <div className="detail-backbar"><button className="back-button" onClick={() => onNavigate('deal-detail')}><ArrowLeft size={15} /> Back to deal intelligence</button><span>Customer Insights / Helios Health System</span></div>
  <PageHeader eyebrow="Revenue Intelligence / Account view" title="Customer Insights" description="Understand account health, stakeholder momentum, expansion signals, and the next best action for every strategic customer." action="Add account note" />
  <div className="insight-grid"><Section title="Helios Health System" icon={Users}><div className="account-heading"><div className="company-mark">HH</div><div><strong>Tier 1 strategic account</strong><small>Healthcare Systems · Mumbai</small></div><span className="status critical">At risk</span></div><div className="account-stats"><div><small>Relationship health</small><strong>68%</strong></div><div><small>Expansion potential</small><strong>₹4.2 Cr</strong></div><div><small>Last executive touch</small><strong>12 days ago</strong></div></div></Section><Section title="Account health" icon={Gauge}><div className="customer-health"><strong>68 / 100</strong><div className="health-meter"><i /></div><small>At risk · stable adoption, cooling executive engagement</small></div><div className="customer-health-stats"><div><small>Renewal confidence</small><strong>76%</strong></div><div><small>Open risks</small><strong>3</strong></div><div><small>Relationship age</small><strong>4.2 yrs</strong></div></div></Section></div>
  <Section title="Active deals" icon={Target} action={<span className="status critical">1 requires attention</span>}><div className="table-wrap"><table><thead><tr><th>Deal</th><th>Stage</th><th>Value</th><th>Health</th><th>Next milestone</th><th>Action</th></tr></thead><tbody><tr><td><button className="deal-link" onClick={() => onNavigate('deal-detail')}><strong>Enterprise Diagnostics Cloud</strong><small>Q-9482 · Clinical deployment</small></button></td><td>Final negotiation</td><td>₹7,47,60,000</td><td><span className="score critical">48 / 100</span></td><td><small>Clinical pilot · 14 days</small></td><td><button className="small-button" onClick={() => onNavigate('deal-detail')}>Open deal</button></td></tr><tr><td><strong>Care Analytics Expansion</strong><small>Q-9361 · Renewal expansion</small></td><td>Solution review</td><td>₹1,84,00,000</td><td><span className="score warning">72 / 100</span></td><td><small>Security review · 32 days</small></td><td><button className="small-button">View</button></td></tr></tbody></table></div></Section>
  <div className="insight-grid"><Section title="Customer signals" icon={Activity}><div className="signal-list"><Signal title="CIO engagement cooling" detail="No response across the last 2 executive check-ins" tone="amber" /><Signal title="Procurement pressure rising" detail="Discount request increased by 4.5 points this week" tone="red" /><Signal title="Clinical sponsor remains active" detail="Pilot team confirmed the next milestone review" /></div></Section><Section title="Deal history" icon={BarChart3}><div className="history-list"><div><span>Q2 2025</span><strong>Platform renewal completed</strong><small>₹2.8 Cr · Closed won</small></div><div><span>Q4 2024</span><strong>Clinical analytics deployment</strong><small>₹1.2 Cr · Closed won</small></div><div><span>Q1 2024</span><strong>Initial enterprise agreement</strong><small>₹3.6 Cr · Closed won</small></div></div></Section></div>
  <Section title="Recommended account actions" icon={Sparkles}><div className="recommendations"><ActionRow number="1" title="Schedule CIO sponsor briefing" detail="Re-anchor the business case around clinical deployment risk." button="Create task" /><ActionRow number="2" title="Share fulfillment recovery plan" detail="Pair the Dallas reallocation with a firm delivery commitment." button="Prepare note" onClick={() => onNavigate('deal-detail')} /><ActionRow number="3" title="Protect renewal path" detail="Use the active pilot milestone to frame the three-year extension." button="Open plan" /></div></Section>
</>; }
>>>>>>> f03bd3b11888b3ca02faaff75269f548307b2463
...
function NegotiationWorkspace({ onNavigate }) {
  ...
  const showFeedback = (message) => setFeedback(message);
  return <>
    <div className="detail-backbar"><button className="back-button" onClick={() => onNavigate('deal-detail')}><ArrowLeft size={15} /> Back to deal intelligence</button><span>Negotiation Intelligence / Helios Health System</span></div>
    <PageHeader eyebrow="Revenue Intelligence / Commercial strategy" title="Negotiation Intelligence" description="Understand the customer position, protect commercial value, and prepare the next response with confidence." action="Open deal rescue" onAction={() => onNavigate('rescue')} />
    {feedback && <div className="negotiation-feedback" role="status"><Check size={15} /> {feedback}</div>}
    <div className="negotiation-overview"><Metric label="Deal value" value="₹7,47,60,000" detail="Enterprise Diagnostics Cloud" badge="Final negotiation" /><Metric label="Current discount" value="21.03%" detail="₹52,275 below list price" badge="Above guardrail" tone="amber" /><Metric label="Current margin" value="58.4%" detail="Target margin is 63.2%" badge="Under pressure" tone="red" /><Metric label="Deal health" value="48 / 100" detail="Critical · action within 24 hours" badge="Critical" tone="red" /></div>
    <div className="negotiation-columns"><Section title="Negotiation overview" icon={Target} action={<span className="status warning">In active review</span>}><div className="negotiation-account"><div className="company-mark">HH</div><div><strong>Helios Health System</strong><small>Tier 1 strategic account · Marcus Vance</small></div></div><div className="negotiation-facts"><div><small>Payment terms</small><strong>Net 45 requested</strong></div><div><small>Current status</small><strong>Counteroffer pending</strong></div><div><small>Deal stage</small><strong>Final negotiation</strong></div></div></Section><Section title="Customer position" icon={Users}><div className="position-block"><span>Customer request</span><strong>Reduce commercial price and preserve the clinical pilot date.</strong><p>Procurement is asking for an additional 4.5% concession and Net 60 terms after the fulfillment delay.</p></div><div className="chips"><span className="soft-tag">Price sensitive</span><span className="soft-tag">Timeline critical</span><span className="soft-tag">Net 60 requested</span></div></Section></div>
    <div className="negotiation-columns"><Section title="Commercial position" icon={CircleDollarSign}><div className="commercial-grid"><div><small>Current quotation</small><strong>₹7,47,60,000</strong></div><div><small>Target price</small><strong>₹7,85,00,000</strong></div><div><small>Minimum acceptable margin</small><strong>58.0%</strong></div><div><small>Maximum discount allowed</small><strong>22.0%</strong></div><div><small>Approval threshold</small><strong>Discount &gt; 18%</strong></div></div></Section><Section title="Recommended response" icon={Sparkles}><div className="recommendation-callout"><span>Protect value, trade certainty</span><strong>Offer 20% discount with a three-year commitment.</strong><p>Pair the concession with a firm reallocation date and keep the warranty addendum tied to the longer term.</p></div><div className="recommendation-facts"><div><small>Expected margin impact</small><strong>58.4% → 59.1%</strong></div><div><small>Approval required</small><strong>Tier 2 commercial</strong></div><div><small>Deal-health impact</small><strong className="success-text">Stabilizes at-risk</strong></div></div></Section></div>
    <Section title="Negotiation history" icon={Activity}><div className="negotiation-timeline"><div><b>1</b><span>Customer request<small>Today · Procurement requested 4.5% additional discount and Net 60.</small></span></div><div><b>2</b><span>Sales response<small>Yesterday · Seller anchored on deployment certainty and current margin.</small></span></div><div><b>3</b><span>Counteroffer<small>Yesterday · Three-year term and warranty addendum proposed.</small></span></div><div><b>4</b><span>Approval event<small>Yesterday · Tier 2 approval opened for commercial exception.</small></span></div><div><b>5</b><span>Latest customer response<small>Today · Customer will review if pilot date is guaranteed.</small></span></div></div></Section>
    <Section title="Reason for recommendation and next action" icon={ShieldCheck}><p className="recommendation-reason">The customer’s urgency is driven by the clinical pilot, while the fulfillment recovery plan gives us a credible non-price concession. Protecting the three-year term preserves account value and keeps the margin above the approved floor.</p><div className="negotiation-actions"><button className="primary-button" onClick={() => showFeedback('Counteroffer draft saved locally')}>Draft counteroffer</button><button className="secondary-button" onClick={() => showFeedback('Tier 2 approval request prepared')}>Request approval</button><button className="secondary-button" onClick={() => showFeedback('Negotiation note saved locally')}>Save negotiation note</button><button className="secondary-button" onClick={() => onNavigate('customer')}>Customer insights</button></div></Section>
  </>;
}

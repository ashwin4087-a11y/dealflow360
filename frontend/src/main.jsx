...
import {
  Activity, AlertTriangle, ArrowRight, BarChart3, Bell, Boxes, Check,
...
function DealDetailPage({ onNavigate }) { return <><PageHeader eyebrow="Revenue Intelligence / Deal detail" title="Deal Intelligence Detail" description="A focused view of the health signals, commercial exposure, and next actions for the selected enterprise deal." action="Open rescue" onAction={() => onNavigate('rescue')} /><Section title="Helios Health System · Enterprise Diagnostics Cloud" icon={Target}><div className="detail-grid"><div><span className="status critical">Critical · 48 / 100</span><h2 className="detail-title">Clinical deployment is blocked by a warehouse shortage</h2><p className="case-copy">OptiSensor Pro-4 is unavailable in Mumbai WH-01, putting the clinical pilot milestone at risk within the next 14 days.</p><div className="detail-meta"><div><small>Deal value</small><strong>₹7,47,60,000</strong></div><div><small>Owner</small><strong>Marcus Vance</strong></div><div><small>Stage</small><strong>Final negotiation</strong></div></div></div><div className="detail-score"><span>Health score</span><strong>48</strong><small>Critical threshold: &lt;50</small></div></div></Section><div className="detail-columns"><Section title="Root-cause signals" icon={AlertTriangle}><div className="signal-list"><Signal title="Warehouse shortage" detail="14-day backorder on OptiSensor Pro-4 in Mumbai." tone="red" /><Signal title="Margin concession pressure" detail="Procurement requested an additional ₹8,16,000 discount." tone="amber" /><Signal title="Stakeholder stagnation" detail="VP of IT has missed the last two check-ins." /></div></Section><Section title="Recommended rescue actions" icon={Zap}><ActionRow number="1" title="Reallocate inventory" detail="Move 6 units from Dallas WH-02 to protect the pilot." button="Open rescue" onClick={() => onNavigate('rescue')} /><ActionRow number="2" title="Defend gross margin" detail="Counter with a three-year warranty addendum." button="Draft offer" /></Section></div></>; }

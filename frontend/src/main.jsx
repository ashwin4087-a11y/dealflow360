  ...
  return <div className="app-shell">
    ...
    <div className="main-shell">
      ...
      <main className="content">
        ...
        {!['health', 'rescue', 'customer', 'negotiation'].includes(page) && <OperationalPage title={active?.[0] || 'Sales Dashboard'} />}
      </main>
    </div>
  </div>;
}
...
function RiskTable({ onNavigate }) { return <Section title="At-risk deals & prescribed interventions" icon={Filter} action={<button className="secondary-button compact">Filter risks</button>}><div className="table-wrap"><table><thead><tr><th>Deal & account</th><th>Total size</th><th>Health score</th><th>Root cause / risk vector</th><th>Prescribed rescue</th><th>Actions</th></tr></thead><tbody>{deals.map((deal) => <tr key={deal.account}><td><strong>{deal.account}</strong><small>{deal.title}</small></td><td>{deal.value}</td><td><span className={`score ${deal.state === 'Critical' ? 'critical' : 'warning'}`}>{deal.score} / 100</span><small>{deal.state}</small></td><td><span className="soft-tag">{deal.reason}</span></td><td><small>{deal.state === 'Critical' ? 'Action within 24h' : 'Action within 48h'}</small></td><td><button className="small-button" onClick={() => onNavigate('rescue')}>{deal.state === 'Critical' ? 'Open rescue' : 'Apply remedy'}</button></td></tr>)}</tbody></table></div></Section>; }
